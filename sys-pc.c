...
char *doUser(char *pc, int ir) {
    ...
    RCASE FOPEN:  y=cpop(); TOS=(cell_t)fopen(CTOS, y);
    RCASE FCLOSE: t1=pop(); fclose((FILE*)t1);
    RCASE FREAD:  t1=pop(); n1=pop(); y = CTOS;
        TOS = feof((FILE*)t1) ? 0 : fread( y, 1, n1, (FILE*)t1);
    RCASE FWRITE: t1=pop(); n1=pop(); TOS = fwrite(CTOS, 1, n1, (FILE*)t1);
