void sysLoad() {
    ...
    parseF(": (lsp) $%x ;", &lsp);
    parseF(": (here) $%x ;", &here);
    parseF(": (vhere) $%x ;", &vhere);
    parseF(": (last) $%x ;", &last);
    parseF(": (stk) $%x ;", &ds.stk[0].i);
    parseF(": (rstk) $%x ;", &rs.stk[0].c);
    parseF(": tib $%x ;", &tib[0]);
    parseF(": >in $%x ;", &in);
    parseF(": mem $%x ;", &mem[0]);
    parseF(": mem-sz %ld ;", MEM_SZ);
    parseF(": vars $%x ;", &vars[0]);
    parseF(": vars-sz %ld ;", VARS_SZ);
    parseF(": regs $%x ;", &reg[0]);
    parseF(": (output_fp) $%x ;", &output_fp);
    parseF(": (input_fp) $%x ;", &input_fp);
    parseF(": state $%x ;", &state);
